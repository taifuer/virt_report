...
日报最多精选 30 条，而不是简单按邮件数量截取：候选线程先按活跃度、补丁/RFC/安全信号和项目配额排序，再保证 QEMU、KVM、Libvirt 的覆盖，并在技术影响相近时优先保留 x86、Arm 议题。已在近 14 期日报出现的线程最多保留 5 条，提示词会要求对比上次摘要说明本次进展，减少连续日报重复。周报和月报上限分别为 27、36 条。
