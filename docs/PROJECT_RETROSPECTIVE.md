...
> 记录范围：2026-07-13—2026-09-01
> 状态：已完成从原型到可持续运行产品的 0→1 建设，部署于 [virt.taifua.com](https://virt.taifua.com/)，并完成第一轮开源准备；GitHub 仓库当前仍为 private。
...
从 2026-07-13 的初始化版本到 2026-09-01 的搜索、自动化和开源准备，项目持续按可独立验证的改动迭代。它已不是一次性生成静态 HTML 的脚本，而是一套可持续运行的虚拟化社区动态系统。
...
- 只检索已点评议题的中英文搜索，并支持项目、类别、架构、排序和分页筛选。
...
- SQLite 幂等存储、游标、线程物化、报告、专题快照、搜索索引、调度记录和会议目录。
- DeepSeek 兼容 API、思考模式、JSON 输出、诊断用降级模板、原子发布和调用量记录。
...
### 2.3 部署初期数据快照（2026-08-01）

以下数值只保留为部署初期记录，不代表当前线上实时数据：
...
此外，KVM Forum 已整理 2010—2025 年议程；学术会议观察覆盖 2010—2026 年，首批公开快照包含 82 篇经人工复核的虚拟化相关论文。
...
- `virt_report/search.py`：已点评议题的离线索引、去重与查询。
...
新增首页、报告页、归档页和关于页，统一命名为 `virt-report`，逐步去掉当时只有外观、没有索引能力的搜索框，以及同步条、深色模式等没有实际价值的元素。文案也从偏宣传式表达改为事实导向，例如“日报 / 周报 / 月报”“统计”“概览”“数据来自社区，分析仅供参考”。
...
Logo 最终采用带轻微侧面透视的三层 V，从小到大表达虚机、虚拟化层与宿主机关系，并由同一高清母版生成标题图标和 Favicon，统一绿色背景、清晰度与尺寸。
...
2026 年 8 月之后的优化重点从“增加页面”转向降低运行和维护风险：

- 报告只有在 AI 成功并通过结构校验后才正式发布；生成、重试和失败状态独立保存。
- 增加基于 SQLite FTS 的中英文搜索，只索引已经进入周期报告的议题，跨日报、周报和月报去重。
- 完善会议论文的来源、单位规范化和人工复核边界，并继续补充 2026 年已公布内容。
- 自动备份保留期调整为 7 天；调度器重启会把上次进程遗留的 `running` 记录关闭为 `interrupted`。
- 增加 Python 3.11/3.12 测试和容器构建 CI，整理软件许可、数据政策与项目文档。
- README 回归项目入口，详细的数据源、使用、报告和部署内容拆入独立文档；仓库尚未切换为 public。
...
没有 API Key 或模型失败时，系统仍可在内部生成降级内容用于诊断，但自动任务不再把它写成公开报告。生成中、等待重试和最终失败保存在独立状态表；首页和归档展示克制的状态提示，RSS、专题和已发布报告计数只消费正式 AI 报告。AI 成功时，正稿写入与状态清理在同一事务完成，避免线上出现“先看到降级稿，稍后又变成 AI 稿”的空档期。

### 7.4 模型升级策略

模型配置集中在 `config.yaml`。2026-08-01 起，新报告统一使用 `deepseek-v4-flash`；历史报告保留生成时内容和模型记录，不为“统一”而改写历史。这既保持可追溯性，也避免重新调用产生不必要成本和结论漂移。

V4 Flash 支持 1M 上下文，但更大的窗口不等于应输入更多噪声。项目保留候选线程预筛，只把单条 opening/latest/review 证据恢复到采集层的完整 600 字；真正导致截断的是思考 token 与 JSON 共用的输出预算，因此将日报、周报、月报分别提高到 32K、48K、64K，JSON 重试最高 96K。多次响应的 usage 必须累加，否则成本统计会低估一次“截断后成功”的实际调用。
...
- 早期只有外观、没有索引能力的搜索框先删除；直到离线索引和内容边界明确后，才重新加入真正可用的搜索。
...
- 搜索只覆盖已点评议题，并作为独立页面放入主导航，不与原始邮件全文检索混淆。
- “查看年度点评”统一为“查看技术演进”。
- 会议论文入口最终统一为更准确的“查看相关论文”。
...
- 任务超时一小时，失败/降级最多重试三次。
- 报告重试状态写入 SQLite，容器重启后继续等待中的任务。
- 达到重试上限时必须先移出内存队列并写终态，避免第三次尝试被无限重复。
- 调度进程重启后主动关闭遗留的 `running` 记录并标记为 `interrupted`，避免运行页长期显示不存在的活动任务。
...
“会议”作为入口，分为 KVM Forum 工程线和学术会议研究线；“查看技术演进”进入年度点评，“查看相关论文”进入可筛选、可分页的论文列表。会议来源默认展开，长期演进概述在移动端也直接显示。
...
| 搜索框有入口但搜不到有效内容 | 只有界面，没有稳定索引和内容边界 | 离线索引已点评议题，跨周期去重 | 搜索应在数据模型准备好以后再上线 |
...
| Logo 与 Favicon 不一致 | 多轮资产独立调整 | 同一绿色底与三层 V 视觉语言 | 品牌资产应共用一个高清视觉源 |
| Git 部署后没有历史数据 | `data/` 正确地未入库 | 备份恢复或冷启动 | 代码发布与运行数据迁移要分开设计 |
| 调度器重启后一直显示运行中 | 上次进程未完成的记录没有终态 | 启动时标记为 `interrupted` | 可观测记录也需要处理进程崩溃语义 |
...
```text
增量采集 → 完整性记录 → 报告生成状态 → 失败重试 → AI 正稿原子发布
...
当前测试以 `pytest` 为主，截至 2026-09-01 项目有 99 个自动化测试。重点覆盖：
...
- 搜索索引去重、中英文查询、筛选、排序和分页。
- 调度器重启后的遗留运行状态清理及其幂等性。
...
- 仓库已完成第一轮开源准备但仍保持 private；正式公开前还需再次核对静态快照的数据边界、文档与线上行为是否一致，不必为了形式增加额外治理文件。
...
10. **最有价值的功能往往来自删减。** 先删除无效搜索、装饰按钮和重复状态，等数据边界明确后再加入真正有用的能力。
