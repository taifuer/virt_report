...
- QEMU、Libvirt、KVM 三项目概览、具体议题、影响判断和后续观察。
- 功能、缺陷、其他分类，以及 x86、ARM 等架构标签和优先展示。
- 热迁移、热升级、热插拔、启动与生命周期、性能、安全与漏洞等专题。
- 首页日/周/月各显示最近 15 期，配合日、周、月切换日历查找历史内容。
...
报告卡片最初占用面积大、空白多，数据达到数百期后难以浏览。最终将日期和报告类型放在同一行，类型字号与日期平衡；首页每类保留 15 期，更多内容进入归档和日历。
...
| 首页数据多后难查 | 大卡片、空白多、无归档 | 紧凑卡片、15 期、归档、日历 | 首页负责发现，不负责承载全部历史 |
